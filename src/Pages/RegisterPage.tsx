...
function HomePage() {
  ...
  const addCategory = () => {
    if (!newCat || !newCatBudget) {
      alert("Please enter a category name and budget amount.");
      return;
    }
    const newCategory = {
          ...
          >
            ...
            <Box sx={{ width: "100%", height: 350 }}>
              {categories.length > 0 ? (
                <BarChart
                  series={[
                    { data: greenData, color: 'green', stack: "total", label: "Money Spent" },
                    { data: yellowData, color: 'gold', stack: "total", label: "Remaining" },
                    { data: redData, color: '#B8382C', stack: "total", label: "Over Budget" },
                  ]}
                  xAxis={[{
                    scaleType: "band",
